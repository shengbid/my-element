...
export const lastRecord = (() => {
  ...
  try {
    if (window.btoa) { // btoa() 方法用于创建一个 base-64 编码的字符串;base-64 解码使用方法是 atob()
      data = atob(data)
    }
...
export const lan = (() => {
  ...
  if (!l && navigator.languages) { // Chrome 32+及Firefox32+版本中，可以通过navigator.languages获取Accept-languages的值
